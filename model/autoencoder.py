...
class Encoder(nn.Module):
    ...
    def __init__(self, *, channels: int, channel_multipliers: List[int], n_resnet_blocks: int,
        ...
        # Initial 3x3 convolution layer that maps the image to channels
        ...
        for i in range(n_resolutions):
            ...
            # Add to list of top-level blocks
        ...
        # Map to embedding space with a 3 x 3 convolution
...
class Decoder(nn.Module):
    ...
    def __init__(self, *, channels: int, channel_multipliers: List[int], n_resnet_blocks: int,
        ...
        # Initial 3 x 3 convolution layer that maps the embedding space to channels
        ...
        # Map to image space with a 3 x 3 convolution
        self.norm_out = normalization(channels)
...
class AttnBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # Compute softmax(Q K^T / sqrt(d_key)) over sequence dimension
        attn = torch.einsum('bci,bcj->bij', q, k) * self.scale
        attn = F.softmax(attn, dim=2)

        # Compute softmax(Q K^T / sqrt(d_key)) · V
...
class DownSample(nn.Module):
    ...
    def __init__(self, channels: int):
        ...
        """
        super().__init__()
        # 3 x 3 convolution with stride length of 2 to down-sample by a factor of 2
